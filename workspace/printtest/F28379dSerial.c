...
__interrupt void RXAINT_recv_ready(void)
{
    ...
    if (RXAdata & 0xC000) {
        ...
        if (RXAdata == 'a') {
            GpioDataRegs.GPBCLEAR.bit.GPIO34 = 1; //if input character is 'a' then turn on the red LED
        }
        if (RXAdata == 'b') {
            GpioDataRegs.GPBSET.bit.GPIO34 = 1; //if input character is 'b' then turn off the red LED
        }
        numRXA ++;
    }
