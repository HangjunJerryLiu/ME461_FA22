...
extern float turn;
extern float Vref;
extern float dis_left;
extern float dis_right;
extern float ninetyturn;
extern int mode; // flag for operation
extern float PL_i; // initial left wheel position when button is pressed
extern float PR_i; // initial right whell position when button is pressed
...
__interrupt void RXAINT_recv_ready(void)
{
    ...
    if (RXAdata & 0xC000) {
        ...
        numRXA ++;
        if (RXAdata == 'a') {       // left turn when a is pressed
            turn = turn + 0.1;
        } else if (RXAdata == 'd') {     // right turn when d is pressed
            turn = turn - 0.1;
        } else if (RXAdata == 'w') {     // speed up when w is pressed
            Vref = Vref + 0.1;
        } else if (RXAdata == 's') { // slow down when s is pressed
            Vref = Vref - 0.1;
        } else if (RXAdata == 'c') {
            turn = 0;
            Vref = Vref;            // stop any turns when c is pressed
        }
        else if (RXAdata == 'q'){    // left turn 90 degrees
            Vref = 0.0;
            PL_i = dis_left;
            PR_i = dis_right;
            mode = 2;

        } else if (RXAdata == 'e'){    // left turn 90 degree
            PL_i = dis_left;
            PR_i = dis_right;
            mode = 3;
        } else if (RXAdata == '1'){    // forward 1 ft
            mode = 1; // mode 1 for go straight 1 ft
            PL_i = dis_left;
            PR_i = dis_right;
        }
        else {
