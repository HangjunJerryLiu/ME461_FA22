...
__interrupt void RXDINT_recv_ready(void)
{
    ...
    if (RXDdata & 0xC000) {
        ...
        if (com_state == 0) { //wait for data from pi
            ...
            com_state = 0; //reset com state after receiving
            ...
            if (RXDdata == '!') { //check the second ! received
                // int i = 0;
                // for (i = 0; i < 2; i++) {
                ...
                serial_sendSCID(&SerialD, crossing_flag[0], 1); //send crossing_flag to pi
                com_state = 0;
            } else {
                com_state = 0;
            }
